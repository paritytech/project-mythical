...
pub mod pallet {
	...
	use frame_support::{
		...
		pallet_prelude::*,
		sp_runtime::traits::{Hash, IdentifyAccount, Verify},
	...
	impl<T: Config> Clone for Batch<T> {
		fn clone(&self) -> Self {
			Self { sender: self.sender.clone(), bias: self.bias.clone(), calls: self.calls.clone() }
		}
	}
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn batch(
			...
			for (i, payload) in batch.calls.into_iter().enumerate() {
				...
				let origin = <T::RuntimeOrigin>::from(frame_system::RawOrigin::Signed(
