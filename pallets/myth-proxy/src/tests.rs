...
mod proxy {
	...
	fn creation_should_store_proxy() {
		new_test_ext().execute_with(|| {
			...
			Balances::make_free_balance_be(&delegator, 10);
	...
	mod sponsorship {
		...
		fn deposit_can_be_sponsored() {
			new_test_ext().execute_with(|| {
				...
				Balances::make_free_balance_be(&sponsor, 10);
		...
		fn agent_can_approve_add_proxy() {
			new_test_ext().execute_with(|| {
				...
				System::assert_last_event(
					Event::ProxySponsorshipApproved { delegator, sponsor, agent: sponsor_agent }
						.into(),
				);
