...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn add_proxy(
			...
			match sponsor.clone() {
				Some(sponsor) => {
					...
					T::Currency::reserve(&sponsor, T::ProxyDeposit::get())?;
				},
				None => {
					T::Currency::reserve(&delegator, T::ProxyDeposit::get())?;
				},
			}
		...
		pub fn proxy(
			...
			let maybe_proxy = Proxies::<T>::get(&address, &delegatee);
			...
			new_origin.add_filter(move |c: &<T as frame_system::Config>::RuntimeCall| {
				...
				match c.is_sub_type() {
					// Proxy call cannot add or remove a proxy.
					Some(Call::add_proxy { .. }) | Some(Call::remove_proxy { .. }) => false,

					_ => proxy_def.proxy_type.filter(c),
				}
			});

			let e = call.dispatch(new_origin);

			e.map_err(|e| e.error)?;

			Self::deposit_event(Event::ProxyExecuted { delegator: address, delegate: delegatee });
		...
		pub fn remove_proxy(origin: OriginFor<T>, delegate: T::AccountId) -> DispatchResult {
			...
			match proxy_def.sponsor {
				Some(sponsor) => {
					T::Currency::unreserve(&sponsor, T::ProxyDeposit::get());
				},
				None => {
					T::Currency::unreserve(&delegator, T::ProxyDeposit::get());
				},
			}
		...
		pub fn approve_proxy_funding(
			...
			let sponsor_agent = ensure_signed(origin)?;

			ensure!(
				Self::has_sponsor_agent(&sponsor, &sponsor_agent),
				Error::<T>::SponsorAgentUnauthorized
			);

			Self::add_approval(&delegator, &sponsor, &sponsor_agent);

			Self::deposit_event(Event::ProxySponsorshipApproved {
				delegator,
				sponsor,
				agent: sponsor_agent,
			});
		...
		///
		/// This extrinsic allows a sponsor to revoke the authorization of an agent. Once revoked, the agent
		/// will no longer be able to approve the reservation of funds for proxies on behalf of the sponsor.
		/// All previously approved fund reservations by this agent will also be invalidated.
		///
...
impl<T: Config> Pallet<T> {
	...
	pub fn has_sponsorship_approval(delegator: &T::AccountId, sponsor: &T::AccountId) -> bool {
		let maybe_agent = SponsorshipApprovals::<T>::get(&(delegator.clone(), sponsor.clone()));

		match maybe_agent {
			Some(agent) => Self::has_sponsor_agent(sponsor, &agent),
			None => false,
		}
	}
